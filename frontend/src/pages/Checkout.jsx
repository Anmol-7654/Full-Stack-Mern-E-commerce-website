...
const Checkout = () => {
  ...
  const bypassPayment = async () => {
    try {
      const saveOrderRes = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          items: cartItems.map((item) => ({
            product: item.id || item.productId || item.product,
      ...
      if (!saveOrderRes.ok) {
        alert(data.message || 'Unable to create the test order.');
        return;
      }
      dispatch(clearCart());
      ...
      console.error(error);
      alert('Unable to reach the backend. Please make sure it is running.');
    }
  };
