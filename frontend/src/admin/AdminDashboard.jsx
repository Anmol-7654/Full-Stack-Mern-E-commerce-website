...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (user.role !== 'admin') {
      setError('Please log in with an administrator account to view dashboard data.');
      return;
    }
    ...
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '5px' }}>
        <img
          ...
          alt="ShopNest logo"
          style={{ height: '56px', width: '56px', borderRadius: '10px', objectFit: 'cover', filter: 'drop-shadow(0 0px 10px rgba(249, 115, 22, 0.3))' }}
        />
        <h2 style={{ margin: 0 }}>Admin Dashboard</h2>
      </div>
      <p style={{ color: '#a1a1aa', marginBottom: '30px', fontSize: '1.1rem' }}>Welcome back, <span style={{color: '#fff'}}>{user?.name}</span></p>
      {user?.role !== 'admin' && (
        <p style={{ color: '#fdba74', marginBottom: '20px' }}>Administrator data and actions require an admin account.</p>
      )}
      {error && <p style={{ color: '#fca5a5', marginBottom: '20px' }}>{error}</p>}
